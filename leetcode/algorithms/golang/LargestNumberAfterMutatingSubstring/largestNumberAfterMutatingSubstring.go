...
func maximumNumber(num string, change []int) string {
    numBytes := []byte(num)  // 将字符串转换为字节切片，以便修改
    shouldReplace := false   // 标记是否已开始替换
    
    for i := range numBytes {
        digit := int(numBytes[i] - '0')  // 将字符转换为数字
        
        if change[digit] > digit {
            // 如果替换后的数字更大，则替换
            numBytes[i] = byte(change[digit] + '0')  // 将数字转换回字符
            shouldReplace = true  // 开始替换
        } else if change[digit] < digit && shouldReplace {
    ...
    return string(numBytes)  // 将字节切片转换回字符串
}
...
func maximumNumber(num string, change []int) string {
    ...
    for i := 0; i < len(numBytes); i++ {
        ...
        if change[digit] > digit {
            ...
            j := i
            for j < len(numBytes) {
                currentDigit := int(numBytes[j] - '0')
                if change[currentDigit] >= currentDigit {
                    // 如果替换后的数字不小于原数字，则替换
                    numBytes[j] = byte(change[currentDigit] + '0')
                    j++
                } else {
                    // 否则停止替换
                    break
